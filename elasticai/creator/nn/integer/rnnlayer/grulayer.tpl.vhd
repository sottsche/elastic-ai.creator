...
entity ${name} is
    ...
    signal gru_cell_y_address : std_logic_vector(log2(Y_COUNT) -1 downto 0);
    ...
    signal y_read_out_address : std_logic_vector(Y_ADDR_WIDTH -1 downto 0);
    signal x_1_addr_int : integer range 0 to X_1_COUNT := 0;
    signal cell_y_store_en : std_logic := '0';
    signal cell_y_store_address : std_logic_vector(Y_ADDR_WIDTH - 1 downto 0);
    signal cell_y_read_addr : std_logic_vector(Y_ADDR_WIDTH - 1 downto 0);
...
begin
    ...
    variable read_out_counter : integer range 0 to Y_COUNT := 0;
    variable var_y_store_counter : integer range 0 to Y_COUNT := 0;
    ...
    if rising_edge(clock) then
            ...
            read_out_counter := 0;
               ...
               if delay = 0 then
                    if read_out_done = '0' then
                        if read_out_counter < Y_COUNT -1 then
        ...
        cell_y_store_addr <= std_logic_vector(to_unsigned(var_y_store_counter, cell_y_store_addr'length));
    end if;
    end process;
