...
from elasticai.creator.nn.integer.mgucell.fnh.design import MGUCell as MGUCellDesign
...
class MGUCell(DesignCreatorModule, nn.Module):
    def __init__(self, **kwargs):
        ...
        self.f_linear = Linear(
            ...
            in_features=self.hidden_size + self.inputs_size,
        ...
        self.fnh_hadamard = HadamardProduct(
            name=self.name + "_fnh_hadamard",
            num_features=self.hidden_size,  # TODO: check this
            num_dimensions=1,  # TODO: check this
            quant_bits=self.quant_bits,
            quant_data_dir=self.quant_data_dir,
    ...
    def create_design(self, name) -> MGUCellDesign:
        return MGUCellDesign(
            ...
            fnh_hadamard=self.fnh_hadamard,
    ...
    def int_forward(
        ...
        q_concatenated_inputs = self.concatenate.int_forward(
            q_inputs1=q_h_prev,
            q_inputs2=q_inputs
        )
        # gate linear transformations and activations
        # update gate
        q_f_linear_outputs = self.f_linear.int_forward(q_inputs=q_concatenated_inputs)
        ...
        q_nh_linear_outputs = self.nh_linear.int_forward(q_inputs=q_h_prev)


        #fnh_hadamard_product
        q_fnh_hadamard_outputs = self.fnh_hadamard.int_forward(
            q_inputs1=q_f_sigmoid_outputs,
        ...
        q_n_linear_outputs = self.n_addition.int_forward(
            ...
            q_inputs2=q_fnh_hadamard_outputs
        )
        #n_tanh
    ...
    def forward(
        ...
        concatenate_outputs = self.concatenate.forward(
            inputs1=inputs,
            inputs2=h_prev,
        ...
        fnh_hadamard_outputs = self.fnh_hadamard.forward(
            inputs1=f_sigmoid_outputs,
            inputs2=nh_linear_outputs,
            given_inputs1_QParams=self.f_sigmoid.outputs_QParams,
            given_inputs2_QParams=self.nh_linear.outputs_QParams
        )
        ...
        n_addition_outputs = self.n_addition.forward(
            ...
            inputs2=fnh_hadamard_outputs,
            given_inputs1_QParams=self.ni_linear.outputs_QParams,
            given_inputs2_QParams=self.fnh_hadamard.outputs_QParams,
        )
